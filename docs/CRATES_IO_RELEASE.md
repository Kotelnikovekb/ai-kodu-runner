...
- Confirm the package name `ai-codu-runer`. Renaming a crates.io package later
  requires a migration and is not automatic.
...
- Reserve/check all four package names: `runner-protocol`, `runner-core`,
  `executor-docker`, and `ai-codu-runer`.
...
cargo package -p runner-protocol --allow-dirty
cargo package -p runner-core --allow-dirty
cargo package -p executor-docker --allow-dirty
cargo package -p ai-codu-runer --allow-dirty
```

Inspect the archive before publishing:

```sh
tar -tzf target/package/ai-codu-runer-0.1.0.crate
```
...
without uploading. The dependency order is `runner-protocol`, `runner-core`,
`executor-docker`, then `ai-codu-runer`.
